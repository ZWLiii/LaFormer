...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # 所有 ConvBNReLU 层都输出 decode_channels 通道数
        ...
        # 添加转换层，将 encode_channels[-3] 转换为 decode_channels
        self.Conv3 = ConvBNReLU(encode_channels[-3], decode_channels, 1)

        # 添加转换层，将 encode_channels[-4] 转换为 decode_channels
        self.Conv4 = ConvBNReLU(encode_channels[-4], decode_channels, 1)

        self.p1 = Fusion(decode_channels)
        self.seg_head = MAF(decode_channels, fc_ratio=fc_ratio, dilation=dilation[3], dropout=dropout, num_classes=num_classes)

        self.init_weight()

    def forward(self, res1, res2, res3, res4, h, w):
        # 调整编码器输出的通道数
        res4 = self.Conv1(res4)  # 512 -> decode_channels
        res3 = self.Conv2(res3)  # 256 -> decode_channels
        res2 = self.Conv3(res2)  # 128 -> decode_channels
        res1 = self.Conv4(res1)  # 64 -> decode_channels
