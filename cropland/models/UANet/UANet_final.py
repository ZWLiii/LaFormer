...
class UANet_final_Res18(nn.Module):
    def __init__(self, channel=32, num_classes=2):  # channel改为中间特征通道数，默认32
        super(UANet_final_Res18, self).__init__()
        # 使用resnet18替换resnet50
        self.backbone = resnet18(pretrained=True)

        # 修改对应的通道数，ResNet18的通道数与ResNet50不同
        self.conv_1 = BasicConv2d(64, channel, 3, 1, 1)    # ResNet18 layer1输出通道: 64 -> channel
        self.conv_2 = nn.Sequential(SEM(64, channel))      # ResNet18 layer2输出通道: 64 -> channel
        self.conv_3 = nn.Sequential(SEM(128, channel))     # ResNet18 layer3输出通道: 128 -> channel
        self.conv_4 = nn.Sequential(SEM(256, channel))     # ResNet18 layer4输出通道: 256 -> channel
        self.conv_5 = nn.Sequential(SEM(512, channel))     # ResNet18 layer5输出通道: 512 -> channel
