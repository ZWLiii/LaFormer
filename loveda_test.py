...
def img_writer(inp):
    ...
    if not mask_id or not mask_id.strip():
        return   # 跳过空路径
    if rgb:
        mask_name_tif = mask_id + '.png'
        mask_tif = label2rgb(mask)       # ← 得到彩色图像
        mask_tif = cv2.cvtColor(mask_tif, cv2.COLOR_RGB2BGR)  # ← 转为 OpenCV 的 BGR 顺序
        cv2.imwrite(mask_name_tif, mask_tif)       # ← 保存为彩色图
    else:
        mask_png = (mask.astype(np.uint8))* 255  # 显示增强
...
def main():
    ...
    with torch.no_grad():
        test_loader = DataLoader(
            ...
            batch_size=2,
            num_workers=4,   # 太大，注释
            pin_memory=True, #GPU开关
            drop_last=False,
        )
        results = []
        for input in tqdm(test_loader):
            # raw_prediction NxCxHxW
            raw_predictions = model(input['img'].cuda())   ##########.cuda()  #模型输出
            ...
            raw_predictions = nn.Softmax(dim=1)(raw_predictions)    #转换为概率图
            predictions = raw_predictions.argmax(dim=1)             #生成 mask（每像素选择概率最高的类别）

            for i in range(raw_predictions.shape[0]):
                mask = predictions[i].cpu().numpy()     #将 mask 转 numpy 保存
                mask_name = image_ids[i]
                mask_type = img_type[i]

                if not mask_name:  # 关键补丁
                    continue
    ...
    t0 = time.time()
    mpp.Pool(processes=mp.cpu_count()).map(img_writer, results)    #保存
    t1 = time.time()
